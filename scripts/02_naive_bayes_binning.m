...
% =======================================================
% CERINȚA 10: ÎNVĂȚARE AUTOMATĂ - NAIVE BAYES (Lab 11)
% Eticheta:   pozitia in teren (Forward / Midfielder / Defender)
% Predictori: goluri, pase decisive, cartonase galbene,
%             minute jucate, inaltime (discretizate)
% =======================================================
disp(' ');
disp('=======================================================');
disp('   INVATARE AUTOMATA - NAIVE BAYES (CERINTA 10)');
disp('=======================================================');
...
% -------------------------------------------------------
% PAS 2: DISCRETIZAREA variabilelor numerice
% Naive Bayes functioneaza mai eficient cu variabile categorice.
% Aplicam binning cu semnificatie sportiva clara 
% -------------------------------------------------------
disp('--- Discretizarea variabilelor numerice ---');

% Goluri
goluri_disc = discretize(date_ml.goals, ...
    [0, 1, 4, 10, Inf], ...
    'categorical', {'Fara_goluri', 'Putine', 'Mediu', 'Multe'});

% Pase decisive
pase_disc = discretize(date_ml.assists, ...
    [0, 1, 4, 8, Inf], ...
    'categorical', {'Fara_pase', 'Putine', 'Mediu', 'Multe'});

% Cartonase galbene
cartg_disc = discretize(date_ml.yellow_cards, ...
    [0, 1, 5, 9, Inf], ...
    'categorical', {'Fara', 'Putine (disciplinat)', 'Normale', 'Multe (probleme de disciplina)'});

% % Minute jucate: cuartile 33%/66% (distributie asimetrica)
% q33_min = quantile(date_ml.minutes_played, 0.33);
% q66_min = quantile(date_ml.minutes_played, 0.66);
% min_disc = discretize(date_ml.minutes_played, ...
%     [-Inf, q33_min, q66_min, Inf], ...
%     'categorical', {'Rezerva', 'Titular_partial', 'Regular Starter'});
%     am comentat asta ca nu-mi placea 

% Minute jucate: praguri bazate pe % din minutele unui sezon complet
% Sezon standard: 38 meciuri x 90 min = 3420 minute posibile
%   < 25%  din sezon (~855 min)  -> rezerva / jucator periferic
%   25-50% din sezon (~1710 min) -> jucator de rotatie
%   50-75% din sezon (~2565 min) -> titular frecvent
%   > 75%  din sezon             -> titular de baza (om de baza)
min_disc = discretize(date_ml.minutes_played, ...
    [0, 855, 1710, 2565, Inf], ...
    'categorical', {'Rezerva', 'Jucator_rotatie', 'Titular_frecvent', 'Titular_de_baza'});

% Verificam daca grupurile sunt echilibrate
% Motivatie: Discretizarea transforma datele brute in categorii cu sens sportiv, 
% facilitand calculul probabilitatilor in modelul Naive Bayes (binning).
disp('Distribuția minutelor pe categorii:');
disp(summary(min_disc));


% Inaltime
inaltime_disc = discretize(date_ml.height, ...
    [0, 175, 183, Inf], ...
    'categorical', {'Scund', 'Inaltime medie', 'Inalt'});

% Asamblam matricea de features categorice
%X_cat = [goluri_disc, pase_disc, cartg_disc, min_disc, inaltime_disc]; a
%dat eroare asa 

X_cat = table(goluri_disc, pase_disc, cartg_disc, min_disc, inaltime_disc);
...
XTrain = X_cat(~idx, :);   YTrain = Y_ml(~idx);
XTest  = X_cat( idx, :);   YTest  = Y_ml( idx);

fprintf('Set antrenare: %d exemple | Set testare: %d exemple\n\n', ...
    sum(~idx), sum(idx));

% -------------------------------------------------------
% PAS 4: Antrenare si evaluare Naive Bayes
% -------------------------------------------------------
disp('--- Antrenare model Naive Bayes ---');
nbModel    = fitcnb(XTrain, YTrain);
nbPred     = predict(nbModel, XTest);
nbAccuracy = sum(nbPred == YTest) / numel(YTest) * 100;
fprintf('Acuratete Naive Bayes: %.2f%%\n\n', nbAccuracy);
...
% Matrice de confuzie
figure('Name', 'Cerinta 10 - Matrice Confuzie Naive Bayes', 'NumberTitle', 'off');
confusionchart(YTest, nbPred, ...
    'Title', sprintf('Matrice de confuzie - Naive Bayes (%.1f%%)', nbAccuracy), ...
    'RowSummary',    'row-normalized', ...
    'ColumnSummary', 'column-normalized');

fprintf('Interpretare:\n');
fprintf('  Naive Bayes a fost antrenat pe date discretizate (binning),\n');
fprintf('  tehnica identica cu cea aplicata pe Titanic in Lab 11.\n');
fprintf('  Modelul prezice pozitia in teren a unui jucator pe baza\n');
fprintf('  statisticilor sale de performanta si a profilului fizic.\n');
